...
const geminiAttackStepSchema = {
    type: Type.OBJECT, properties: {
        ...
        mitre_techniques: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Relevant MITRE ATT&CK Technique names." },
...
5.  **Be Realistic:** All commands, tool outputs, and system alerts must be technically accurate.
...
export const getMitreExplanation = async (
  ...
  type: 'tactic' | 'technique'
    ...
    const provider = JSON.parse(localStorage.getItem('adversary_ai_provider') || '"gemini"');
    const prompt = `Provide a concise, one-sentence explanation for the following MITRE ATT&CK ${type}: "${term}". Focus on the high-level goal or action it represents.`;

    try {
        if (provider === 'openai') {
            const apiKey = JSON.parse(localStorage.getItem('adversary_openai_api_key') || '""');
            if (!apiKey) throw new Error("OpenAI API key not found. Please set it in settings.");
